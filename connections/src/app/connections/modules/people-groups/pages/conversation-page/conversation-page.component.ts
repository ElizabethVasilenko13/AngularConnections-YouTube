...
import { conversationBackendSelector, isConversationLoadinSelector, selectConversationById, usersSelector } from '../../store/users/users.selectors';
...
export class ConversationPageComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    sendMessage(): void {
      const message: string = this.usersService.createMessageForm.get('text')?.value;
  ...
  subscribeToConversationData(): void {
    const conversationsDataSubscr = this.conversationData$?.subscribe((data) => {
